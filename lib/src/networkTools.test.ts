...
describe("upsert vnet", () => {
  ...
  it("can create when not found", async () => {
    ...
    vi.spyOn(networkClient.virtualNetworks, "beginCreateOrUpdateAndWait").mockImplementationOnce(
      (groupName, name, vnet) =>
        Promise.resolve({
          ...vnet,
  ...
  it("can update existing", async () => {
    ...
    vi.spyOn(networkClient.virtualNetworks, "get").mockResolvedValue({
      name: vnetName,
      location,
      addressSpace: { addressPrefixes: ["10.22.0.0/16"] },
    });
    vi.spyOn(networkClient.virtualNetworks, "beginCreateOrUpdateAndWait").mockImplementationOnce(
      (groupName, name, vnet) =>
        Promise.resolve({
...
describe("upsert nsg", () => {
  ...
  it("can create via SDK", async () => {
    ...
    vi.spyOn(networkClient.networkSecurityGroups, "beginCreateOrUpdateAndWait").mockImplementationOnce(
      (groupName, name, nsg) =>
        Promise.resolve({
          ...nsg,
...
describe("upsert private zone", () => {
  ...
  it("can create via SDK", async () => {
    ...
    vi.spyOn(privateDnsClient.privateZones, "beginCreateOrUpdateAndWait").mockImplementationOnce(
      (groupName, name, zone) =>
        Promise.resolve({
          ...zone,
  ...
  it("can no-op existing via SDK", async () => {
    ...
    expect(result.name).toBe(zoneName);
    expect(result.location).toBe("Global");
    expect(privateDnsClient.privateZones.get).toHaveBeenCalledExactlyOnceWith(groupName, zoneName, expect.anything());
