...
const mapPins = locations
  ...
  .map(loc => ({
    ...
    name: loc.name,
    lat: parseFloat(loc.metadata!.latitude!),
    lon: parseFloat(loc.metadata!.longitude!),
...
const mapPinsByCell = mapPins.map(pin => ({
  lat: Math.round(pin.lat / cellSize),
  lon: Math.round(pin.lon / cellSize),
}));
const title = " Available Location Map ";
const titlePadding = Math.max(cellCount - title.length, 0);
console.log(
  `+-${"-".repeat(Math.round(titlePadding / 2))}${title}${"-".repeat(titlePadding - Math.round(titlePadding / 2))}-+`,
);
